...
define('DB_HOST', 'localhost');		// Database server hostname (for Windows, or for remote connections - use instead of socket)
define('DB_PORT', '');		// Database server port (blank for default)
define('DB_USER', 'nillbye');		// Database server username
define('DB_PASS', 'xJZ9MnhTW8');		// Database server password
define('DB_NAME', 'nillbye_game');		// Database schema

define('TABLE_PREFIX', '');	// Table name prefix (in case only a single database schema is available)

define('GAME_VERSION', '4.81');
define('GAME_TITLE', 'Nillbyeia '. GAME_VERSION);				// server title, may customize to liking
define('URL_BASE', 'http://www.nillbyelabs.com/game');			// the path in which Promisance is installed
define('URL_HOMEPAGE', 'http://www.nillbyelabs.com');				// where users will be sent when they logout
define('URL_FORUMS', 'http://bit.ly/1byjPZ9');	// where your site's forums are located
										// (comment out if you have none)
define('MAIL_ADMIN', 'admin@nillbyelabs.com');					// administrative contact address
define('MAIL_VALIDATE', 'no-reply@nillbyelabs.com');				// source address for validation emails
define('TXT_NEWS', '<span style="color:white">Working on some cool stuff!</span>');
...
define('TURNS_STORED', 1000);		// Max stored turns
...
define('TURNS_FREQ', .3);	// how often to give turns
...
define('TURNS_CRONTAB', FALSE);	// use "turns.php" to give out turns, scheduled via crontab; otherwise, trigger on page loads
define('TURNS_CRONLOG', FALSE);	// if TURNS_CRONTAB is disabled, store turn logs in the database for retrieval by turns.php
...
define('CLAN_MINJOIN', 12);	// Empires can't leave clans until they've been a member for this many hours
define('CLAN_MINREJOIN', 1);	// Empires can't create/join a new clan until this many hours after they left
define('CLAN_MINSHARE', 2);	// Unsharing clan forces takes this many hours to take effect
define('CLAN_MINRELATE', 1);	// How long a clan must wait before it can modify an alliance or war slot
...
define('BONUS_TURNS', FALSE);	// Allow users to collect 1 hour worth of bonus turns each day.
